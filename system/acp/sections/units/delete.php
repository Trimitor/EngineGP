<?php
	...
	if(isset($url['delete']) AND $url['delete'] == 'all')
	{
		...
		while($server = $sql->get($servers))
		{
			...
			{
				$sql->query('DELETE FROM `help__dialogs` WHERE `help`="'.$help['id'].'"');
				$sql->query('DELETE FROM `help` WHERE `id`="'.$help['id'].'" LIMIT 1');
			}

			$sql->query('DELETE FROM `admins__'.$server['game'].'` WHERE `server`="'.$server['id'].'" LIMIT 1');
			$sql->query('DELETE FROM `address__buy` WHERE `server`="'.$server['id'].'"');
			$sql->query('DELETE FROM `logs__sys` WHERE `server`="'.$server['id'].'"');
