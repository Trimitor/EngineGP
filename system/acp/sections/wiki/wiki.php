<?php
	...
	$sql->query('SELECT `text` FROM `wiki__answer` WHERE `wiki`="'.$id.'" LIMIT 1');
	...
	if($go)
	{
		...
		$sql->query('SELECT `id` FROM `wiki__category` WHERE `id`="'.$aData['cat'].'" LIMIT 1');
		if(!$sql->num())
		...
		$sql->query('UPDATE `wiki__answer` set '
			.'`cat`="'.$aData['cat'].'",'
	...
	$sql->query('SELECT `id`, `name` FROM `wiki__category` ORDER BY `id` ASC');
	while($cat = $sql->get())
