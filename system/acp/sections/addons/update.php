<?php
	...
	$sql->query('SELECT * FROM `plugins__update` WHERE `id`="'.$id.'" LIMIT 1');
	$plugin = $sql->get();
	...
	if($go)
	{
		...
		$sql->query('DELETE FROM `plugins__config` WHERE `update`="'.$id.'"');
		...
		foreach($aData['config_files_file'] as $i => $file)
		{
			...
			$sql->query('INSERT INTO `plugins__config` set `plugin`="'.$plugin['plugin'].'", `update`="'.$id.'", `file`="'.$file.'", `sort`="'.$n.'"');
		}

		if($n)
			$aData['cfg'] = 1;

		$sql->query('DELETE FROM `plugins__clear` WHERE `update`="'.$id.'"');
		...
		foreach($aData['config_clear_file'] as $i => $file)
		{
			...
			$sql->query('INSERT INTO `plugins__clear` set `plugin`="'.$plugin['plugin'].'", `update`="'.$id.'", `text`="'.$text.'", `file`="'.$file.'", `regex`="'.$regex.'"');
		}

		$sql->query('DELETE FROM `plugins__write` WHERE `update`="'.$id.'"');
		...
		foreach($aData['config_write_file'] as $i => $file)
		{
			...
			$sql->query('INSERT INTO `plugins__write` set `plugin`="'.$plugin['plugin'].'", `update`="'.$id.'", `text`="'.$text.'", `file`="'.$file.'", `top`="'.$top.'"');
		}

		$sql->query('DELETE FROM `plugins__write_del` WHERE `update`="'.$id.'"');
		...
		foreach($aData['config_write_del_file'] as $i => $file)
		{
			...
			$sql->query('INSERT INTO `plugins__write_del` set `plugin`="'.$plugin['plugin'].'", `update`="'.$id.'", `text`="'.$text.'", `file`="'.$file.'", `top`="'.$top.'"');
		}

		$sql->query('DELETE FROM `plugins__delete` WHERE `update`="'.$id.'"');
		...
		foreach($aData['files_delete_file'] as $file)
		{
			...
			$sql->query('INSERT INTO `plugins__delete` set `plugin`="'.$plugin['plugin'].'", `update`="'.$id.'", `file`="'.$file.'"');
		}

		if($aData['delete'])
		{
			$sql->query('DELETE FROM `plugins__delete_ins` WHERE `update`="'.$id.'" LIMIT 1');

			$sql->query('INSERT INTO `plugins__delete_ins` set `plugin`="'.$plugin['plugin'].'", `update`="'.$id.'", `file`="'.$aData['delete'].'"');
		}

		$sql->query('UPDATE `plugins__update` set '
	...
	$html->get('update', 'sections/addons');
		...
		$sql->query('SELECT `name` FROM `plugins__category` WHERE `id`="'.$plugin['cat'].'" LIMIT 1');
		$cat = $sql->get();
		...
		$sql->query('SELECT `id`, `file`, `sort` FROM `plugins__config` WHERE `update`="'.$id.'" ORDER BY `sort` ASC');
		while($data = $sql->get())
		{
		...
		$sql->query('SELECT `id`, `text`, `file`, `regex` FROM `plugins__clear` WHERE `update`="'.$id.'" ORDER BY `id` ASC');
		while($data = $sql->get())
		{
		...
		$sql->query('SELECT `id`, `text`, `file`, `top` FROM `plugins__write` WHERE `update`="'.$id.'" ORDER BY `id` ASC');
		while($data = $sql->get())
		{
		...
		$sql->query('SELECT `id`, `text`, `file`, `top` FROM `plugins__write_del` WHERE `update`="'.$id.'" ORDER BY `id` ASC');
		while($data = $sql->get())
		{
			$top = $data['top'] ? 'checked' : '';
		...
		$sql->query('SELECT `id`, `file` FROM `plugins__delete` WHERE `update`="'.$id.'" ORDER BY `id` ASC');
		while($data = $sql->get())
		{
