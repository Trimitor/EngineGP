<?php
	...
	if(isset($url['get']))
	{
		if($url['get'] == 'cat')
		{
			...
			$sql->query('SELECT `id`, `name` FROM `plugins__category` WHERE `game`="'.$game.'" ORDER BY `sort` ASC');
			while($cat = $sql->get())
	...
	if($go)
	{
		...
		if($sql->num())
		{
			$sql->query('INSERT INTO `plugins__update` set `plugin`="0", `name`="'.htmlspecialchars($aData['name']).'", `status`="'.$aData['status'].'", `cfg`="0", `upd`="0"');
		...
		if(!$aData['update'])
		{
			...
			{
				$sql->query('DELETE FROM `plugins__update` WHERE `id`="'.$aData['update'].'" LIMIT 1');
		...
		if(!$aData['aecfg'])
		{
			...
			foreach($aData['config_files_file'] as $i => $file)
			{
				...
				$sql->query('INSERT INTO `plugins__config` set `plugin`="'.$id.'", `update`="'.$aData['update'].'", `file`="'.$file.'", `sort`="'.$n.'"');
			}
		...
		foreach($aData['config_clear_file'] as $i => $file)
		{
			...
			$sql->query('INSERT INTO `plugins__clear` set `plugin`="'.$id.'", `update`="'.$aData['update'].'", `text`="'.$text.'", `file`="'.$file.'", `regex`="'.$regex.'"');
		}
		...
		foreach($aData['config_write_file'] as $i => $file)
		{
			...
			$sql->query('INSERT INTO `plugins__write` set `plugin`="'.$id.'", `update`="'.$aData['update'].'", `text`="'.$text.'", `file`="'.$file.'", `top`="'.$top.'"');
		}
		...
		foreach($aData['config_write_del_file'] as $i => $file)
		{
			...
			$sql->query('INSERT INTO `plugins__write_del` set `plugin`="'.$id.'", `update`="'.$aData['update'].'", `text`="'.$text.'", `file`="'.$file.'", `top`="'.$top.'"');
		}
		...
		foreach($aData['files_delete_file'] as $file)
		{
			...
			$sql->query('INSERT INTO `plugins__delete` set `plugin`="'.$id.'", `update`="'.$aData['update'].'", `file`="'.$file.'"');
		}

		if($aData['delete'])
			$sql->query('INSERT INTO `plugins__delete_ins` set `plugin`="'.$id.'", `update`="'.$aData['update'].'", `file`="'.$aData['delete'].'"');
		...
		if($aData['update'])
		{
			...
			$sql->query('UPDATE `plugins__update` set `upd`="'.$aData['update'].'" WHERE `id`!="'.$aData['update'].'" AND `plugin`="'.$id.'" AND `upd`="0" ORDER BY `id` DESC LIMIT 1');
		}else
