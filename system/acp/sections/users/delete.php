<?php
	...
	if(isset($url['delete']) AND $url['delete'] == 'all')
	{
		...
		$uploads = $sql->query('SELECT `id`, `name` FROM `help__upload` WHERE `user`="'.$id.'"');
		while($upload = $sql->get($uploads))
		{
			@unlink(ROOT.'upload/'.$upload['name']);

			$sql->query('DELETE FROM `help__upload` WHERE `id`="'.$upload['id'].'" LIMIT 1');
		}

		$sql->query('DELETE FROM `logs__sys` WHERE `user`="'.$id.'"');
		$sql->query('DELETE FROM `owners` WHERE `user`="'.$id.'"');
		$sql->query('DELETE FROM `promo__use` WHERE `user`="'.$id.'"');
