<?php
	...
	if($id)
	{
		$sql->query('SELECT * FROM `jobs__app` WHERE `id`="'.$id.'" LIMIT 1');
		$jobs_app = $sql->get();
		...
		if($go)
		{
			...
			$sql->query('UPDATE `jobs__app` set'
				.'`user`="'.$aData['user'].'",'
				.'`text`="'.$aData['text'].'",'
		...
		$sql->query('SELECT * FROM `jobs__app` ORDER BY `id` ASC');
		while($jobs = $sql->get())
		{
			$status = [
				'1' => 'Доступна',
			...
			if(isset($url['del']))
			{
				$sql->query('DELETE FROM `jobs__app` WHERE `id`="'.$url['del'].'" LIMIT 1');
				sys::outjs(array('s' => 'ok'));
