<?php
	...
	class control_scan_servers_admins extends cron
	{
		function __construct()
		{
			...
			{
				$sql->query('SELECT `uid` FROM `control__servers` WHERE `id`="'.$id.'" LIMIT 1');
				$server = $sql->get();

				$admins = $sql->query('SELECT `id`, `text` FROM `control__admins_'.$game.'` WHERE `server`="'.$id.'" AND `active`="1" AND `time`<"'.$start_point.'"');
				...
				while($admin = $sql->get($admins))
				{
					...
					$sql->query('UPDATE `admins__'.$game.'` set `active`="0" WHERE `id`="'.$admin['id'].'" LIMIT 1');
				}
