	...
	class graph_servers_day extends cron
	{
		function __construct()
		{
			...
			while($server = $sql->get($servers))
			{
				...
				$sql->query('SELECT `online`, `cpu`, `ram`, `hdd` FROM `graph__hour` WHERE `server`="'.$server['id'].'" AND `time`>"'.($start_point-86400).'" ORDER BY `id` DESC LIMIT 24');
				...
				$sql->query('INSERT INTO `graph__day` set `server`="'.$server['id'].'",'
