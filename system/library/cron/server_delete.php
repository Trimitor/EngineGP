<?php
	...
	class server_delete extends cron
	{
		function __construct()
		{
			...
			$sql->query('SELECT `id`, `aid` FROM `address__buy` WHERE `server`="'.$server['id'].'" LIMIT 1');
			if($sql->num())
			{
				$add = $sql->get();

				$sql->query('UPDATE `address` set `buy`="0" WHERE `id`="'.$add['aid'].'" LIMIT 1');
				$sql->query('DELETE FROM `address__buy` WHERE `id`="'.$add['id'].'" LIMIT 1');
			}
			...
			$sql->query('DELETE FROM `admins__'.$server['game'].'` WHERE `server`="'.$server['id'].'"');
			$sql->query('DELETE FROM `plugins__install` WHERE `server`="'.$server['id'].'"');
			$sql->query('DELETE FROM `owners` WHERE `server`="'.$server['id'].'"');
			$sql->query('DELETE FROM `graph` WHERE `server`="'.$server['id'].'"');
			$sql->query('DELETE FROM `graph__day` WHERE `server`="'.$server['id'].'"');
			$sql->query('DELETE FROM `graph__hour` WHERE `server`="'.$server['id'].'"');
			$sql->query('DELETE FROM `servers` WHERE `id`="'.$server['id'].'" LIMIT 1');

			$sql->query('INSERT INTO `logs__sys` set `user`="0", `server`="'.$argv[3].'", `text`="Удаление игрового сервера #'.$argv[3].' ('.$server['game'].') unit: #'.$server['unit'].', tarif: #'.$server['tarif'].', slots: #'.$server['slots'].'", `time`="'.$start_point.'"');
