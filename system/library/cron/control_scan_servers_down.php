<?php
	...
	class control_scan_servers_down extends cron
	{
		function __construct()
		{
			...
			{
				$sql->query('SELECT `uid`, `address`, `status`, `autorestart` FROM `control__servers` WHERE `id`="'.$id.'" LIMIT 1');
				$server = $sql->get();
				...
				$sql->query('INSERT INTO `logs__sys` set `user`="0", `server`="'.$id.'", `text`="[Контроль] Перезагрука сервера: сервер завис", `time`="'.$start_point.'"');
			}
