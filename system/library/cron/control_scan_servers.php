<?php
	...
	class control_scan_servers extends cron
	{
		function __construct()
		{
			...
			$sql->query('SELECT `unit` FROM `control__servers` WHERE `id`="'.$servers[5].'" LIMIT 1');
			$server = $sql->get();
			...
			{
				$sql->query('SELECT `uid`, `address`, `status`, `stop` FROM `control__servers` WHERE `id`="'.$id.'" LIMIT 1');
				$server = $sql->get();
					...
					case 'working': case 'change': case 'start': case 'restart':
						if(!sys::int($ssh->get('ps aux | grep s_'.$server['uid'].' | grep -v grep | awk \'{print $2}\'')))
						{
							...
							if($server['stop'])
							{
								...
								$sql->query('INSERT INTO `logs__sys` set `user`="0", `server`="'.$id.'", `text`="[Контроль] Включение сервера: сервер выключен не через панель", `time`="'.$start_point.'"');
							}
						}else
