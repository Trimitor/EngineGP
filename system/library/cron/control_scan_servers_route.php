	...
	class control_scan_servers_route extends cron
	{
		function __construct()
		{
			...
			foreach($aCpu as $cpu => $data)
			{
				...
				$sql->query('SELECT `id` FROM `control__servers` WHERE `unit`="'.$server['unit'].'" AND `core_fix`="'.$core.'" AND `core_fix`="1" LIMIT 1');
				if($sql->num())
				{
					unset($aCpu[$cpu]);
			...
			foreach($uses as $use_core => $use)
			{
				...
				$sql->query('SELECT `id`, `uid` FROM `control__servers` WHERE `unit`="'.$server['unit'].'" AND `game`="'.$game.'" AND `core_use`="'.$use_core.'" AND `status`="working" AND `core_fix`="0" ORDER BY `slots_start` DESC, `online` DESC LIMIT 3');
				if($sql->num() > 1)
				{
					$server = $sql->get();
					...
					$sql->query('UPDATE `control__servers` set `core_use`="'.$core.'" WHERE `id`="'.$server['id'].'" LIMIT 1');
				}
			}
