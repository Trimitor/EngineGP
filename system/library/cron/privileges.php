<?php
	...
	class privileges extends cron
	{
		function __construct()
		{
			...
			{
				$sql->query('SELECT `id` FROM `privileges__buy` WHERE `server`="'.$id.'" AND `status`="1" LIMIT 1');
				if(!$sql->num())
					unset($servers[$i]);
			}
			...
			{
				$sql->query('DELETE FROM `privileges__buy` WHERE `date`<"'.$time.'" AND status`="0" LIMIT 5');
				...
				$privileges = $sql->query('SELECT `id`, `text`, `sql`, `mail` FROM `privileges__buy` WHERE `server`="'.$id.'" AND `status`="1" LIMIT 3');
				while($privilege = $sql->get($privileges))
				{
					$text .= base64_decode($privilege['text']).PHP_EOL;

					$sql->query(base64_decode($privilege['sql']));
					$sql->query('DELETE FROM `privileges__buy` WHERE `id`="'.$privilege['id'].'" LIMIT 1');
