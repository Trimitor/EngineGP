	...
	class sys
	{
		...
		public static function outhtml($text, $time = 3, $url = false, $cache = false)
		{
			...
			$html->get('out');
				...
				$html->set('home', $cfg['http']);
				$html->set('css', $design.$tpl.'\/css/');
				$html->set('js', $design.$tpl.'\/js/');
				$html->set('img', $design.$tpl.'\/images/');
				$html->set('text', $text);
		...
		public static function mail($name, $text, $mail)
		{
			...
			$text = str_replace(
				...
				array($cfg['name'], $text, $cfg['http'], $design.'images/', $design.'css/'),
				$tpl
			);
		...
		public static function cpu_get_idle($first, $second, $unit, $fcpu, $ctrl)
		{
			...
			foreach($threads as $idle)
			{
				...
				if($ctrl)
					$sql->query('SELECT `id` FROM `control__servers` WHERE `unit`="'.$unit.'" AND `core_fix`="'.($core+1).'" LIMIT 1');
				else
