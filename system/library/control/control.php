		...
		public static function iptables($id, $action, $source, $dest, $unit, $snw = false, $ssh = false)
		{
			...
			switch($action)
			{
				case 'block':
					...
					$sql->query('SELECT `id` FROM `control__firewall` WHERE `sip`="'.$source.'" AND `server`="'.$id.'" LIMIT 1');

					// Если такое правило уже добавлено или указан адрес сайта (ПУ)
					if($sql->num() || ($source == $cfg['ip'] || $source == $cfg['subnet']))
						return array('s' => 'ok');

					$sql->query('INSERT INTO `control__firewall` set `sip`="'.$source.'", `dest`="'.$dest[0].':'.$dest[1].'", `server`="'.$id.'", `time`="'.$start_point.'"');
				...
				case 'unblock':
					...
					{
						$sql->query('SELECT `id`, `sip` FROM `control__firewall` WHERE `id`="'.$source.'" AND `server`="'.$id.'" LIMIT 1');

						// Если такое правило отсутствует
						if(!$sql->num())
							return array('s' => 'ok');
					}else{
						$sql->query('SELECT `id`, `sip` FROM `control__firewall` WHERE `sip`="'.$source.'" AND `server`="'.$id.'" LIMIT 1');
					...
					$sql->query('DELETE FROM `control__firewall` WHERE `id`="'.$firewall['id'].'" LIMIT 1');

					return array('s' => 'ok');

				case 'remove':
					$sql->query('SELECT `id`, `sip`, `dest` FROM `control__firewall` WHERE `server`="'.$id.'"');
					...
					$sql->query('DELETE FROM `control__firewall` WHERE `server`="'.$id.'" LIMIT '.$nRule);
