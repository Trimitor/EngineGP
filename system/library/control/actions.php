<?php
	...
	class actions
	{
		public static function stop($id)
		{
			...
			$sql->query('SELECT `uid`, `unit`, `game`, `address`, `name` FROM `control__servers` WHERE `id`="'.$id.'" LIMIT 1');
			$server = $sql->get();
			...
			$sql->query('UPDATE `control__servers` set `status`="off", `online`="0", `players`="", `stop`="0" WHERE `id`="'.$id.'" LIMIT 1');
		...
		public static function change($id, $map = false)
		{
			...
			$design = $cfg['http'].'template/'.$cfg['design'].'/';
			...
			$sql->query('SELECT `uid`, `unit`, `game`, `online`, `players`, `name` FROM `control__servers` WHERE `id`="'.$id.'" LIMIT 1');
			$server = $sql->get();
			...
			if($map)
			{
				...
				// Обновление информации в базе
				$sql->query('UPDATE `control__servers` set `status`="change" WHERE `id`="'.$id.'" LIMIT 1');
			...
			foreach($aMaps as $map)
			{
				$html->get('change_list', 'sections/control/servers/games');
					$html->set('img', file_exists(DIR.'/maps/'.$server['game'].'/'.$map.'.jpg') ? $cfg['http'].'maps/'.$server['game'].'/'.$map.'.jpg' : $design.'images/status/none.jpg');
					$html->set('name', $map);
		...
		public static function reinstall($id)
		{
			...
			$sql->query('SELECT `uid`, `unit`, `address`, `game`, `name`, `pack`, `ftp`, `core_fix` FROM `control__servers` WHERE `id`="'.$id.'" LIMIT 1');
			$server = $sql->get();
			...
			$sql->query('DELETE FROM `control__admins_'.$server['game'].'` WHERE `server`="'.$id.'"'); // Список админов на сервере
			$sql->query('DELETE FROM `control__plugins_install` WHERE `server`="'.$id.'"'); // Список установленных плагинов на сервере

			$core = !isset($core) ? 0 : $core+1;

			// Обновление информации в базе
			$sql->query('UPDATE `control__servers` set `status`="reinstall", `core_use`="'.$core.'", `fastdl`="0" WHERE `id`="'.$id.'" LIMIT 1');
		...
		public static function update($id)
		{
			...
			$sql->query('SELECT `uid`, `unit`, `address`, `game`, `name`, `pack`, `ftp` FROM `control__servers` WHERE `id`="'.$id.'" LIMIT 1');
			$server = $sql->get();
			...
			// Обновление информации в базе
			$sql->query('UPDATE `control__servers` set `status`="update", `core_use`="'.$core.'" WHERE `id`="'.$id.'" LIMIT 1');
		...
		public static function delete($id)
		{
			...
			$sql->query('SELECT `uid`, `unit`, `game`, `slots`, `address` FROM `control__servers` WHERE `id`="'.$id.'" LIMIT 1');
			$server = $sql->get();
			...
			$scopy = $sql->query('SELECT `id`, `name` FROM `control__copy` WHERE `server`="'.$id.'"');
			while($copy = $sql->get($scopy))
			{
				$copys .= 'rm /copy/'.$copy['name'].'.tar;';

				$sql->query('DELETE FROM `control__copy` WHERE `id`="'.$copy['id'].'" LIMIT 1');
			}
			...
			$crons = $sql->query('SELECT `id`, `cron` FROM `control__crontab` WHERE `server`="'.$id.'"');
			while($cron = $sql->get($crons))
			{
				...
				$sql->query('DELETE FROM `control__crontab` WHERE `id`="'.$cron['id'].'" LIMIT 1');
			}

			// Удаление различной информации игрового сервера
			$sql->query('DELETE FROM `control__admins_'.$server['game'].'` WHERE `server`="'.$id.'" LIMIT 1');
			$sql->query('DELETE FROM `control__plugins_install` WHERE `server`="'.$id.'" LIMIT 1');
			$sql->query('DELETE FROM `control__plugins_buy` WHERE `server`="'.$id.'" LIMIT 1');
			$sql->query('DELETE FROM `control__servers` WHERE `id`="'.$id.'" LIMIT 1');
