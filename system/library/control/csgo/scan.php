<?php
	...
	class scan extends scans
	{
		public static function mon($id, $players_get = false)
		{
			...
			$sql->query('SELECT `unit`, `game`, `name`, `map`, `online`, `players`, `status` FROM `control__servers` WHERE `id`="'.$id.'" LIMIT 1');
			$server = $sql->get();
			...
			$sql->query('UPDATE `control__servers` set '
				.'`name`="'.$out['name'].'", '
				.'`online`="'.$out['online'].'", '
				.'`map`="'.$info['map'].'", '
			...
			if($players_get)
				$sql->query('UPDATE `control__servers` set `players`="'.base64_encode($out['players']).'" WHERE `id`="'.$id.'" LIMIT 1');	
		...
		public static function info($sq, $id, $pl = false)
		{
			...
			$sql->query('SELECT `address` FROM `control__servers` WHERE `id`="'.$id.'" LIMIT 1');
			$server = $sql->get();
