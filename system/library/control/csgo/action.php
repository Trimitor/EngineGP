<?php
	...
	class action extends actions
    {
        public static function start($id, $type = 'start')
        {
			...
			$sql->query('SELECT `uid`, `unit`, `game`, `address`, `slots`, `name`, `tickrate`, `map_start`, `vac`, `time_start`, `core_fix`, `pingboost` FROM `control__servers` WHERE `id`="'.$id.'" LIMIT 1');
			$server = $sql->get();
			...
			$sql->query('UPDATE `control__servers` set `status`="'.$type.'", `online`="0", `players`="", `core_use`="'.$core.'", `time_start`="'.$start_point.'", `stop`="1" WHERE `id`="'.$id.'" LIMIT 1');
        ...
        public static function change($id, $map = false)
        {
			...
			foreach($aMaps as $map)
			{
				...
				$html->get('change_list', 'sections/control/servers/csgo');
					...
					$html->set('img', file_exists(DIR.'/maps/'.$server['game'].'/'.$name.'.jpg') ? $cfg['http'].'maps/'.$server['game'].'/'.$name.'.jpg' : $design.'images/status/none.jpg');
        ...
        public static function update($id)
        {
			...
			$sql->query('SELECT `uid`, `unit`, `game`, `name`, `ftp`, `core_fix` FROM `control__servers` WHERE `id`="'.$id.'" LIMIT 1');
			$server = $sql->get();
