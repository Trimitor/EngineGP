<?php
	...
	class plugins
	{
		...
		public static function required($id, $required, $choice, $mcache)
		{
			...
			{
				$sql->query('SELECT `id` FROM `control__plugins_install` WHERE `server`="'.$sid.'" AND `plugin`="'.$pl.'" LIMIT 1');
				if(!$sql->num())
				{
						...
						foreach($aChoice as $plugins)
						{
							...
							if(in_array($pl, $aPlugins))
							{
								...
								foreach($aPlugins as $plugin)
								{
									...
									if($data['upd'])
									{
										$sql->query('SELECT `name` FROM `plugins__update` WHERE `plugin`="'.$plugin.'" ORDER BY `id` DESC LIMIT 1');
										$data = $sql->get();
									}
