<?php
	...
	class scans
	{
		...
		public static function resources($id)
		{
			...
			$sql->query('SELECT `uid`, `unit`, `game`, `slots`, `status`, `online`, `hdd_use` FROM `control__servers` WHERE `id`="'.$id.'" LIMIT 1');
			...
			$sql->query('UPDATE `control__servers` set `ram_use`="'.$resources['ram'].'", `cpu_use`="'.$resources['cpu'].'", `hdd_use`="'.$resources['hdd'].'" WHERE `id`="'.$id.'" LIMIT 1');
		...
		public static function status($id)
		{
			...
			$sql->query('SELECT `uid`, `unit`, `game`, `address`, `status`, `name`, `online`, `players` FROM `control__servers` WHERE `id`="'.$id.'" LIMIT 1');
			$server = $sql->get();
			...
			switch($server['status'])
			{
				case 'working': case 'change': case 'start': case 'restart':
					...
					{
						$sql->query('UPDATE `control__servers` set `status`="off", `online`="0", `players`="0" WHERE `id`="'.$id.'" LIMIT 1');
				...
				case 'off':
					if(sys::int($ssh->get('ps aux | grep s_'.$server['uid'].' | grep -v grep | awk \'{print $2}\'')))
					{	
						$sql->query('UPDATE `control__servers` set `status`="working" WHERE `id`="'.$id.'" LIMIT 1');
				...
				case 'reinstall':
					...
					{
						$sql->query('UPDATE `control__servers` set `status`="off" WHERE `id`="'.$id.'" LIMIT 1');
