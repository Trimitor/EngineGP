<?php
	...
	class actions
	{
		...
		public static function reinstall($id)
		{
			...
			$sql->query('SELECT `uid`, `unit`, `tarif`, `address`, `game`, `name`, `pack`, `plugins__use`, `ftp`, `reinstall`, `core_fix` FROM `servers` WHERE `id`="'.$id.'" LIMIT 1');
			$server = $sql->get();
			...
			$sql->query('SELECT `path`, `install`, `plugins__install` FROM `tarifs` WHERE `id`="'.$server['tarif'].'" LIMIT 1');
			$tarif = $sql->get();
			...
			$sql->query('DELETE FROM `admins__'.$server['game'].'` WHERE `server`="'.$id.'"'); // Список админов на сервере
			$sql->query('DELETE FROM `plugins__install` WHERE `server`="'.$id.'"'); // Список установленных плагинов на сервере
			...
			if($server['plugins_use'])
			{
				...
				if(isset($aPlugins[$server['pack']]))
				{
					...
					foreach($plugins as $plugin)
						if($plugin)
							$sql->query('INSERT INTO `plugins__install` set `server`="'.$id.'", `plugin`="'.$plugin.'", `time`="'.$start_point.'"');
				}
			}
			...
			// Логирование
			$sql->query('INSERT INTO `logs__sys` set `user`="'.$user['id'].'", `server`="'.$id.'", `text`="'.sys::text('syslogs', 'reinstall').'", `time`="'.$start_point.'"');
		...
		public static function update($id)
		{
			...
			$sql->query('SELECT `uid`, `unit`, `tarif`, `address`, `game`, `name`, `pack`, `plugins__use`, `ftp`, `update` FROM `servers` WHERE `id`="'.$id.'" LIMIT 1');
			$server = $sql->get();
			...
			// Логирование
			$sql->query('INSERT INTO `logs__sys` set `user`="'.$user['id'].'", `server`="'.$id.'", `text`="'.sys::text('syslogs', 'update').'", `time`="'.$start_point.'"');
