<?php
	...
	if($id)
	{
		...
		if($update['upd'])
		{
			$sql->query('SELECT '.$sqlq.' FROM `plugins__update` WHERE `id`="'.$update['upd'].'" LIMIT 1');
		...
		$sql->query('SELECT `id`, `file` FROM `plugins__config` WHERE (`plugin`="'.$id.'" AND `update`="0") OR (`plugin`="'.$id.'" AND `update`="'.$update['upd'].'") ORDER BY `sort`, `id` ASC');
	...
	if(!isset($html->arr['main']))
	{
		...
		if($mcache->get('plugins_list_view_'.$url['game']) != '')
			...
			// Категории
			$cats = $sql->query('SELECT `id`, `name` FROM `plugins__category` WHERE `game`="'.$url['game'].'" ORDER BY `sort` ASC');
			while($cat = $sql->get($cats))
			{
				...
				while($plugin = $sql->get($plugins))
				{
					...
					if($plugin['upd'])
					{
						...
						$sql->query('SELECT `name`, `desc`, `images`, `status`, `packs` FROM `plugins__update` WHERE `plugin`="'.$plugin['id'].'" ORDER BY `id` DESC LIMIT 1');
						if($sql->num())
						{
							$plugin = $sql->get();
