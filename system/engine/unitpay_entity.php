<?php
	...
	{
		$sql->query('SELECT `id`, `server`, `price` FROM `privileges__buy` WHERE `key`="'.$params['account'].'" LIMIT 1');
		if(!$sql->num())
		...
		$sql->query('UPDATE `privileges__buy` set `status`="1" WHERE `id`="'.$privilege['id'].'" LIMIT 1');
