<?php
	...
	if($id)
	{
		...
		if($go)
		{
			$sql->query('SELECT `id` FROM `jobs__app` WHERE `user`="'.$user['id'].'" AND `job`="'.$id.'" LIMIT 1');
			if($sql->num())
			...
			$sql->query('INSERT INTO `jobs__app` set'
				.'`user`="'.$user['id'].'",'
				.'`text`="",'
		...
		$sql->query('SELECT `text` FROM `jobs__app` WHERE `user`="'.$user['id'].'" AND `job`="'.$jobs['id'].'" LIMIT 1');
		$text = $sql->get();
