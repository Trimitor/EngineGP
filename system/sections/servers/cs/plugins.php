		...
		if($mcache->get('server_plugins_'.$id) != '')
			...
			while($cat = $sql->get($cats))
			{
				...
				while($plugin = $sql->get($plugins))
				{
					...
					$sql->query('SELECT `id` FROM `plugins__install` WHERE `server`="'.$id.'" AND `plugin`="'.$plugin['id'].'" LIMIT 1');
					if($sql->num())
						continue;
					...
					if($plugin['price'])
					{
						$sql->query('SELECT `id` FROM `plugins__buy` WHERE `plugin`="'.$plugin['id'].'" AND `server`="'.$id.'" LIMIT 1');
						$buy = $sql->num();
					}
			...
			$pl_ins = $sql->query('SELECT `plugin`, `upd`, `time` FROM `plugins__install` WHERE `server`="'.$id.'" ORDER BY `plugin`');
