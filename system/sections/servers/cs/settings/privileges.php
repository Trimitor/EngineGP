<?php
	...
	if(isset($url['active']))
	{
		...
		if($sql->num())
		{
			...
			if(!$privilege['active'])
			{
				$sql->query('SELECT `id` FROM `privileges__list` WHERE `server`="'.$id.'" LIMIT 1');
				if(!$sql->num())
	...
	if(isset($url['delete']))
	{
		$sql->query('DELETE FROM `privileges__list` WHERE `id`="'.sys::int($url['delete']).'" AND `server`="'.$id.'" LIMIT 1');
	...
	if($go)
	{
		$sql->query('SELECT `id` FROM `privileges__list` WHERE `server`="'.$id.'" LIMIT 10');
		if($sql->num() > 9)
		...
		$sql->query('INSERT INTO `privileges__list` set `server`="'.$id.'", `name`="'.$aData['name'].'", `flags`="'.$aData['flags'].'", `data`="'.sys::b64js($data).'"');
	...
	$sql->query('SELECT `id`, `name`, `flags`, `data` FROM `privileges__list` WHERE `server`="'.$id.'" ORDER BY `id` ASC LIMIT 10');
