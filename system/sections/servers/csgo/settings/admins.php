<?php
	...
	if($go)
	{
		...
		$sql->query('DELETE FROM `admins__'.$server['game'].'` WHERE `server`="'.$id.'"');
		...
		foreach($aData['value'] as $index => $val)
		{
			if($val != '')
			{
				...
				$sql->query('INSERT INTO `admins__'.$server['game'].'` set'
					.'`server`="'.$id.'",'
	...
	$sql->query('SELECT `id`, `value`, `active`, `passwd`, `flags`, `immunity`, `time`, `info` FROM `admins__'.$server['game'].'` WHERE `server`="'.$id.'" ORDER BY `id` ASC');
	while($admin = $sql->get())
	{
	...
	$sql->query('SELECT `id` FROM `admins__'.$server['game'].'` WHERE `server`="'.$id.'" ORDER BY `id` DESC LIMIT 1');
	$max = $sql->get();
