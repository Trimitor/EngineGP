<?php
	...
	foreach($sPlugins as $index => $plugins)
	{
		while($plugin = $sql->get($plugins))
		{
			...
			if($sUpdate[$index])
			{
				$sql->query('SELECT `id`, `upd`, `time` FROM `plugins__install` WHERE `server`="'.$id.'" AND `plugin`="'.$plugin['plugin'].'" LIMIT 1');

				$aPlugins[] = $plugin['plugin'];
			}else{
				$sql->query('SELECT `id`, `upd`, `time` FROM `plugins__install` WHERE `server`="'.$id.'" AND `plugin`="'.$plugin['id'].'" LIMIT 1');
