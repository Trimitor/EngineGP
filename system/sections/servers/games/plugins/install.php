<?php
	...
	if($plugin['upd'])
	{
		$sql->query('SELECT `name`, `id`, `cfg`, `incompatible`, `choice`, `required`, `packs`, `price` FROM `plugins__update` WHERE `plugin`="'.$pid.'" ORDER BY `id` DESC LIMIT 1');
		if($sql->num())
		{
	...
	{
		// Проверка покупки
		$sql->query('SELECT `id` FROM `plugins__buy` WHERE `plugin`="'.$pid.'" AND `server`="'.$id.'" LIMIT 1');
		if($sql->num())
			$buy = true;
		else{
			// Проверка баланса
	...
	if(!$buy AND $plugin['price'])
	{
		...
		$sql->query('INSERT INTO `plugins__buy` set `plugin`="'.$pid.'", `key`="'.md5(strip_tags($plugin['name'])).'", `server`="'.$id.'", `price`="'.$plugin['price'].'", `time`="'.$start_point.'"');
	...
	$sql->query('INSERT INTO `plugins__install` set `server`="'.$id.'", `plugin`="'.$pid.'", `upd`="'.$upd.'", `time`="'.$start_point.'"');
