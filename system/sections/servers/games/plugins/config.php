<?php
	...
	$sql->query('SELECT `plugin`, `update`, `file` FROM `plugins__config` WHERE `id`="'.$fid.'" LIMIT 1');
	...
	$sql->query('SELECT `id` FROM `plugins__install` WHERE `server`="'.$id.'" AND `plugin`="'.$config['plugin'].'" LIMIT 1');
	...
	if($config['update'])
		$sql->query('SELECT `name` FROM `plugins__update` WHERE `id`="'.$config['update'].'" LIMIT 1');
	else
