<?php
	...
	$sql->query('SELECT `id` FROM `plugins__install` WHERE `server`="'.$id.'" AND `plugin`="'.$pid.'" LIMIT 1');
	if(!$sql->num())
		exit();

	// Проверка установки обновления плагина
	$sql->query('SELECT `id` FROM `plugins__install` WHERE `server`="'.$id.'" AND `plugin`="'.$pid.'" AND `upd`="'.$plugin['id'].'" LIMIT 1');
	if($sql->num())
	...
	$sql->query('UPDATE `plugins__install` set `upd`="'.$plugin['id'].'", `time`="'.$start_point.'" WHERE `server`="'.$id.'" AND `plugin`="'.$pid.'" LIMIT 1');
