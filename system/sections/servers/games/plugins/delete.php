<?php
	...
	$sql->query('SELECT `id`, `upd` FROM `plugins__install` WHERE `server`="'.$id.'" AND `plugin`="'.$pid.'" LIMIT 1');
	if(!$sql->num())
	...
	$sql->query('DELETE FROM `plugins__install` WHERE `server`="'.$id.'" AND `plugin`="'.$pid.'"');
