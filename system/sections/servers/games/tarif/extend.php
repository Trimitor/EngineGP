<?php
	...
	if($go)
	{
		...
		if($add_sum)
			$sql->query('UPDATE `address__buy` set `time`=`time`+"2592000" WHERE `server`="'.$id.'" LIMIT 1');
		...
		if(!is_array($promo))
			...
			$sql->query('INSERT INTO `promo__use` set `promo`="'.$promo['id'].'", `user`="'.$user['id'].'", `time`="'.$start_point.'"');
