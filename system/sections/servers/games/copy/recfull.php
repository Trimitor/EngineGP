<?php
	...
	// Удаление плагинов
	$sql->query('DELETE FROM `plugins__install` WHERE `server`="'.$id.'"');
	...
	foreach($aPlugins as $plugin)
	{
		...
		$sql->query('SELECT `id` FROM `plugins__install` WHERE `plugin`="'.$aPlugin[0].'" AND `server`="'.$id.'" LIMIT 1');

		if(!$aPlugin[1])
			$aPlugin[1] = 0;

		if(!$sql->num())
			$sql->query('INSERT INTO `plugins__install` set `server`="'.$id.'", `plugin`="'.$aPlugin[0].'", `upd`="'.$aPlugin[1].'", `time`="'.$copy['date'].'"');
	}
