<?php
	...
	foreach($aPlugin as $plugin)
	{
		...
		$sql->query('SELECT `id` FROM `plugins__install` WHERE `plugin`="'.$plugin.'" AND `server`="'.$id.'" LIMIT 1');

		if(!$sql->num())
			$sql->query('INSERT INTO `plugins__install` set `server`="'.$id.'", `plugin`="'.$plugin.'", `time`="'.$copy['date'].'"');
	}
