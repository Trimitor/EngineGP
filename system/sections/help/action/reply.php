<?php
	...
	if(is_array($aData['images']) AND count($aData['images']))
	{
		foreach($aData['images'] as $img)
		{
			...
			$sql->query('SELECT `id` FROM `help__upload` WHERE `name`="'.$img.'" LIMIT 1');
			if(!$sql->num())
				continue;

			$image = $sql->get();

			$sql->query('UPDATE `help__upload` set `status`="1" WHERE `id`="'.$image['id'].'" LIMIT 1');
	...
	if($user['group'] == 'user')
	{
		$i = 0;
		$n = 3;
		$sql->query('SELECT `user` FROM `help__dialogs` WHERE `help`="'.$id.'" ORDER BY `id` DESC LIMIT 3');
		while($msg = $sql->get())
		{
			if(!$i AND !$msg['user'])
		...
		if(!$n)
		{
			$sql->query('INSERT INTO `help__dialogs` set `help`="'.$id.'", `user`="0", `text`="Пожалуйста, дождитесь ответа технической поддержки.", `img`="", `time`="'.$start_point.'"');

			sys::outjs(array('i' => 'Пожалуйста, дождитесь ответа технической поддержки.'), $nmch);
		}
	}

	$sql->query('SELECT `text` FROM `help__dialogs` WHERE `help`="'.$id.'" ORDER BY `id` DESC LIMIT 1');
	$msg = $sql->get();

	if(md5($msg['text']) == md5($aData['text']))
		sys::outjs(array('e' => 'Такое сообщение уже отправлено.'), $nmch);

	$sql->query('INSERT INTO `help__dialogs` set '
		.'`help`="'.$id.'",'
		.'`user`="'.$user['id'].'",'
		.'`text`="'.$aData['text'].'",'
