<?php
	...
	if($id)
	{
		...
		$dialogs = $sql->query('SELECT `id`, `img` FROM `help__dialogs` WHERE `help`="'.$id.'"');
		while($dialog = $sql->get($dialogs))
		{
			...
			foreach($aImg as $img)
			{
				$sql->query('DELETE FROM `help__upload` WHERE `name`="'.$img.'" LIMIT 1');

				unlink(ROOT.'upload/'.$img);
			}

			$sql->query('DELETE FROM `help__dialogs` WHERE `id`="'.$dialog['id'].'" LIMIT 1');
		}
