<?php
	...
	if($go)
	{
		...
		$sql->query('SELECT `id` FROM `admins__'.$server['game'].'` WHERE `server`="'.$server['id'].'" AND `value`="'.htmlspecialchars($aData['data']).'" LIMIT 1');
		if($sql->num())
		...
		$sql->query('SELECT `flags`, `immunity`, `data` FROM `privileges__list` WHERE `id`="'.$aData['service'].'" AND `server`="'.$server['id'].'" LIMIT 1');
		if(!$sql->num())
		...
		if($server['game'] == 'cs')
		{
			...
			$sqlq = 'INSERT INTO `admins__'.$server['game'].'` set'
					.'`server`="'.$server['id'].'",'
		...
		$sql->query('SELECT `key` FROM `privileges__buy` WHERE '
		...
		if(!$sql->num())
		{
			...
			$sql->query('INSERT INTO `privileges__buy` set '
					.'`server`="'.$server['id'].'",'
	...
	if(isset($url['select']))
	{
		if($url['select'] == 'time')
		{
			...
			$sql->query('SELECT  `data` FROM `privileges__list` WHERE `id`="'.$service.'" LIMIT 1');
			$list = $sql->get();
		...
		$sql->query('SELECT  `id`, `name` FROM `privileges__list` WHERE `server`="'.$server['id'].'" ORDER BY `id` ASC LIMIT 5');
		while($list = $sql->get())
			$name .= '<option value="'.$list['id'].'">'.$list['name'].'</option>';

		$sql->query('SELECT  `data` FROM `privileges__list` WHERE `server`="'.$server['id'].'" ORDER BY `id` ASC LIMIT 1');
		$list = $sql->get();
