<?php
	...
	if(in_array($ctrl['status'], array('install', 'overdue', 'blocked')))
		...
		if($go)
		{
			...
			$sql->query('SELECT `id` FROM `control__servers` WHERE `unit`="'.$id.'" LIMIT '.$ctrl['limit']);
			...
			{
				$sql->query('SELECT `id` FROM `control__servers` WHERE `unit`="'.$id.'" AND `address`="'.$ip.':'.$port_min.'" LIMIT 1');
				if(!$sql->num())
				{
					$port = $port_min;
			...
			$sql->query('INSERT INTO `control__servers` set '
				.'`unit`="'.$id.'",'
				.'`address`="'.$ip.':'.$port.'",'
				.'`game`="'.$game.'",'
				.'`slots`="32",'
			...
			$sql->query('UPDATE `control__servers` set `uid`="'.$uid.'" WHERE `id`="'.$id.'" LIMIT 1');
