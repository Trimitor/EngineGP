<?php
	...
	if($go)
	{
		...
		if(isset($url['rating']))
		{
			...
			$sql->query('SELECT `id` FROM `boost__rating` WHERE `boost`="'.$aData['site'].'" AND `user`="'.$user['id'].'" AND `rating`="'.$rating.'" LIMIT 1');
			if($sql->num())
				sys::out('err');

			$sql->query('DELETE FROM `boost__rating` WHERE `boost`="'.$aData['site'].'" AND `user`="'.$user['id'].'" LIMIT 1');
			$sql->query('INSERT INTO `boost__rating` set `boost`="'.$aData['site'].'", `rating`="'.$rating.'", `user`="'.$user['id'].'"');

			$sql->query('SELECT SUM(`rating`) FROM `boost__rating` WHERE `boost`="'.$aData['site'].'"');
			$sum = $sql->get();
		...
		$sql->query('INSERT INTO `control__boost` set `user`="'.$user['id'].'", `server`="'.$sid.'", `site`="'.$aData['site'].'", `circles`="'.$aBoost[$server['game']][$aData['site']]['circles'][$aData['service']].'", `money`="'.$sum.'", `date`="'.$start_point.'"');
    ...
    if($mcache->get('ctrl_server_boost_'.$sid) != '')
        ...
        $html->get('boost', 'sections/control/servers/'.$server['game']);
			...
			{
				$sql->query('SELECT SUM(`rating`) FROM `boost__rating` WHERE `boost`="'.$boost.'"');
				$sum = $sql->get();
