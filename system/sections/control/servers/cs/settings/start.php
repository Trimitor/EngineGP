<?php
	...
	$sql->query('SELECT `uid`, `slots`, `map_start`, `vac`, `fps`, `fastdl`, `autorestart`, `pingboost`, `core_fix` FROM `control__servers` WHERE `id`="'.$sid.'" LIMIT 1');
		...
		switch($url['save'])
		{
			...
			case 'fps':
				if(in_array($value, array('300', '500', '1100')))
					$sql->query('UPDATE `control__servers` set `fps`="'.$value.'" WHERE `id`="'.$sid.'" LIMIT 1');
