<?php
	...
	if(isset($url['subsection']) AND in_array($url['subsection'], $aSub))
	{
		...
		if($mcache->get('ctrl_server_plugins_'.$sid) != '')
			...
			// Категории
			$cats = $sql->query('SELECT `id`, `name` FROM `plugins__category` WHERE `game`="'.$server['game'].'" ORDER BY `sort` ASC');
			while($cat = $sql->get($cats))
			{
				...
				while($plugin = $sql->get($plugins))
				{
					...
					$sql->query('SELECT `id` FROM `control__plugins_install` WHERE `server`="'.$sid.'" AND `plugin`="'.$plugin['id'].'" LIMIT 1');
					if($sql->num())
						continue;
					...
					if($plugin['upd'])
					{
						...
						$sql->query('SELECT `name`, `desc`, `images`, `status`, `packs`, `price` FROM `plugins__update` WHERE `plugin`="'.$plugin['id'].'" ORDER BY `id` DESC LIMIT 1');
						if($sql->num())
						{
							$plugin = $sql->get();
					...
					if($plugin['price'])
					{
						$sql->query('SELECT `id` FROM `plugins__buy` WHERE `plugin`="'.$plugin['id'].'" AND `server`="'.$sid.'" LIMIT 1');
						$buy = $sql->num();
					}
			...
			$pl_ins = $sql->query('SELECT `plugin`, `upd`, `time` FROM `control__plugins_install` WHERE `server`="'.$sid.'" ORDER BY `plugin`');
			while($plugin = $sql->get($pl_ins))
			{
				...
				if($isUpd)
					$sql->query('SELECT `name`, `desc`, `status`, `cfg`, `upd` FROM `plugins__update` WHERE `id`="'.$isUpd.'" LIMIT 1');
				else
