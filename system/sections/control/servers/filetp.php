<?php
	...
	$sql->query('SELECT `uid`, `address`, `game`, `status` FROM `control__servers` WHERE `id`="'.$sid.'" LIMIT 1');
	$server = $sql->get();
	...
	if(end($frouter) == 'noaccess.php')
		...
		$sql->query('SELECT `uid`, `ftp`, `ftp_passwd` FROM `control__servers` WHERE `id`="'.$sid.'" LIMIT 1');
			...
			if(isset($url['action']))
				switch($url['action'])
				{
					case 'on':
						...
						$sql->query('UPDATE `control__servers` SET `ftp`="1", `ftp_passwd`="'.$server['ftp_passwd'].'" WHERE `id`="'.$sid.'" LIMIT 1');
					...
					case 'change':
						...
						$sql->query('UPDATE `control__servers` SET `ftp_passwd`="'.$passwd.'" WHERE `id`="'.$sid.'" LIMIT 1');
					...
					case 'off':
						...
						$sql->query('UPDATE `control__servers` SET `ftp`="0" WHERE `id`="'.$sid.'" LIMIT 1');
