<?php
	...
	$sql->query('SELECT `id` FROM `plugins__update` WHERE `plugin`="'.$pid.'" ORDER BY `id` DESC LIMIT 1');
	...
	$sql->query('SELECT `id` FROM `control__plugins_install` WHERE `server`="'.$sid.'" AND `plugin`="'.$pid.'" LIMIT 1');
	if(!$sql->num())
		exit();

	// Проверка установки обновления плагина
	$sql->query('SELECT `id` FROM `control__plugins_install` WHERE `server`="'.$sid.'" AND `plugin`="'.$pid.'" AND `upd`="'.$plugin['id'].'" LIMIT 1');
	if($sql->num())
		sys::outjs(array('e' => 'Данный плагин уже обновлен'));

	// Данные обновления
	$sql->query('SELECT `id`, `cfg`, `incompatible`, `required` FROM `plugins__update` WHERE `id`="'.$plugin['id'].'" LIMIT 1');
	...
	// Удаление файлов
	$sql->query('SELECT `file` FROM `plugins__delete` WHERE `update`="'.$plugin['id'].'"');
	...
	$sql->query('SELECT `text`, `file`, `regex` FROM `plugins__clear` WHERE `update`="'.$plugin['id'].'"');
	while($clear = $sql->get())
	...
	$sql->query('SELECT `text`, `file`, `top` FROM `plugins__write` WHERE `update`="'.$plugin['id'].'" ORDER BY `id` ASC');
	while($write = $sql->get())
		plugins::write($write, $server['uid'], $dir);

	// Обновление данных в базе
	$sql->query('UPDATE `control__plugins_install` set `upd`="'.$plugin['id'].'", `time`="'.$start_point.'" WHERE `server`="'.$sid.'" AND `plugin`="'.$pid.'" LIMIT 1');
