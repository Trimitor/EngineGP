<?php
	...
	if($install['upd'])
		$sql->query('SELECT `name`, `info`, `images`, `upd` FROM `plugins__update` WHERE `id`="'.$install['upd'].'" LIMIT 1');
	else
	...
	if($mcache->get('ctrl_server_plugin_'.$pid.$sid) != '')
		...
		$sql->query('SELECT `id`, `file` FROM `plugins__config` WHERE (`plugin`="'.$pid.'" AND `update`="0") OR (`plugin`="'.$pid.'" AND `update`="'.$install['upd'].'") ORDER BY `sort`, `id` ASC');
