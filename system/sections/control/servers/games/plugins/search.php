<?php
	...
	if(!$sql->num($plugins))
	{
		$plugins = $sql->query('SELECT `id`, `plugin`, `packs` FROM `plugins__update` WHERE `game`="'.$server['game'].'" AND (`name` LIKE FROM_BASE64(\''.base64_encode('%'.$text.'%').'\') OR `desc` LIKE FROM_BASE64(\''.base64_encode('%'.$text.'%').'\')) AND `upd`="0" LIMIT 5');
		$update = true;
	}
	...
	if(!$sql->num($plugins))
	{
		...
		if(strpos($text, ' '))
		{
			...
			foreach($aText as $word)
			{
				...
				if(!$sql->num($plugins))
				{
					$plugins = $sql->query('SELECT `id`, `plugin`, `packs` FROM `plugins__update` WHERE (`name` LIKE FROM_BASE64(\''.base64_encode('%'.$word.'%').'\') OR `desc` LIKE FROM_BASE64(\''.base64_encode('%'.$word.'%').'\')) AND `upd`="0" LIMIT 5');
					$update = true;
				}
	...
	foreach($sPlugins as $index => $plugins)
	{
		while($plugin = $sql->get($plugins))
		{
			...
			if($sUpdate[$index])
			{
				$sql->query('SELECT `id`, `upd`, `time` FROM `control__plugins_install` WHERE `server`="'.$sid.'" AND `plugin`="'.$plugin['plugin'].'" LIMIT 1');

				$aPlugins[] = $plugin['plugin'];
			}else{
				$sql->query('SELECT `id`, `upd`, `time` FROM `control__plugins_install` WHERE `server`="'.$sid.'" AND `plugin`="'.$plugin['id'].'" LIMIT 1');
			...
			if($upd)
				$sql->query('SELECT `name`, `desc`, `status`, `cfg`, `upd` FROM `plugins__update` WHERE `id`="'.$upd.'" LIMIT 1');
			else
			...
			$html->get('search', 'sections/control/servers/games/plugins');
				...
				if($install)
				{
					...
					$sql->query('SELECT `name`, `desc`, `status`, `cfg` FROM `plugins__update` WHERE `plugin`="'.$plugin['id'].'" AND `upd`="0" LIMIT 1');
					if($sql->num())
					{
						$upd = $sql->get();
