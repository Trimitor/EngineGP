<?php
	...
	if($plugin['upd'])
	{
		$sql->query('SELECT `name`, `id`, `cfg`, `incompatible`, `required`, `packs`, `price` FROM `plugins__update` WHERE `plugin`="'.$pid.'" ORDER BY `id` DESC LIMIT 1');
		if($sql->num())
		{
	...
	{
		// Проверка покупки
		$sql->query('SELECT `id` FROM `control__plugins_buy` WHERE `plugin`="'.$pid.'" AND `server`="'.$sid.'" LIMIT 1');
		if($sql->num())
			$buy = true;
		else{
			// Проверка баланса
	...
	// Удаление файлов
	$sql->query('SELECT `file` FROM `plugins__delete` '.$qsql);
	while($delete = $sql->get())
		$ssh->set('sudo -u server'.$server['uid'].' rm '.$dir.$delete['file']);

	// Удаление текста из файлов
	$sql->query('SELECT `text`, `file`, `regex` FROM `plugins__clear` '.$qsql);
	while($clear = $sql->get())
		plugins::clear($clear, $server['uid'], $dir);

	// Добавление текста в файлы
	$sql->query('SELECT `text`, `file`, `top` FROM `plugins__write` '.$qsql);
	while($write = $sql->get())
	...
	if(!$buy AND $plugin['price'])
	{
		...
		$sql->query('INSERT INTO `control__plugins_buy` set `plugin`="'.$pid.'", `key`="'.md5(strip_tags($plugin['name'])).'", `server`="'.$sid.'", `price`="'.$plugin['price'].'", `time`="'.$start_point.'"');
	...
	$sql->query('INSERT INTO `control__plugins_install` set `server`="'.$sid.'", `plugin`="'.$pid.'", `upd`="'.$upd.'", `time`="'.$start_point.'"');
