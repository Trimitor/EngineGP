<?php
	...
	$sql->query('SELECT `id`, `upd` FROM `control__plugins_install` WHERE `server`="'.$sid.'" AND `plugin`="'.$pid.'" LIMIT 1');
	if(!$sql->num())
	...
	$sql->query('SELECT `text`, `file` FROM `plugins__write` '.$qsql);
	while($clear = $sql->get())
	...
	$sql->query('SELECT `text`, `file`, `top` FROM `plugins__write_del` '.$qsql);
	while($write = $sql->get())
		plugins::write($write, $server['uid'], $dir);

	// Удаление записи установленного плагина в базе
	$sql->query('DELETE FROM `control__plugins_install` WHERE `server`="'.$sid.'" AND `plugin`="'.$pid.'"');

	// Очистка кеша
	$mcache->delete('ctrl_server_plugins_'.$id);

	if($plugin['upd'])
		$sql->query('SELECT `install` FROM `plugins__delete_ins` WHERE `update`="'.$plugin['upd'].'" LIMIT 1');
	else
		$sql->query('SELECT `install` FROM `plugins__delete_ins` WHERE `plugin`="'.$pid.'" AND `update`="0" LIMIT 1');
