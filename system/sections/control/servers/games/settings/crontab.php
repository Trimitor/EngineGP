<?php
	...
	if($go)
	{
		...
		if(isset($url['action']) AND $url['action'] == 'delete')
		{
			...
			$sql->query('SELECT `cron` FROM `control__crontab` WHERE `id`="'.$task.'" AND `server`="'.$sid.'" LIMIT 1');
			if(!$sql->num())
			...
			$sql->query('DELETE FROM `control__crontab` WHERE `id`="'.$task.'" LIMIT 1');

			sys::outjs(array('s' => 'ok'), $nmch);
		}

		// Добавление задания
		$sql->query('SELECT `id` FROM `control__crontab` WHERE `server`="'.$sid.'" LIMIT 5');
		...
		$sql->query('INSERT INTO `control__crontab` set `server`="'.$sid.'"');
		$cid = $sql->id();
		...
		$sql->query('UPDATE `control__crontab` set `server`="'.$sid.'", `task`="'.$data['task'].'", `cron`="'.$cron_rule.'", `week`="'.$week.'", `time`="'.$time.'", `commands`="'.$data['commands'].'" WHERE `id`="'.$cid.'" LIMIT 1');
	...
	$sql->query('SELECT `id`, `task`, `week`, `time` FROM `control__crontab` WHERE `server`="'.$sid.'" ORDER BY `id` ASC');
