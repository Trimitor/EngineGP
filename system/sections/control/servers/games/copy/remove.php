<?php
	...
	$sql->query('SELECT `name`, `status` FROM `control__copy` WHERE `id`="'.$cid.'" AND `user`="'.$ctrl['user'].'_'.$id.'" AND `game`="'.$server['game'].'" LIMIT 1');
	if(!$sql->num())
	...
	$sql->query('DELETE FROM `control__copy` WHERE `id`="'.$cid.'" LIMIT 1');
