<?php
	...
	$sql->query('SELECT `id`, `pack`, `name`, `plugins`, `date`, `status` FROM `control__copy` WHERE `id`="'.$cid.'" AND `user`="'.$ctrl['user'].'_'.$id.'" AND `game`="'.$server['game'].'" LIMIT 1');
	if(!$sql->num())
	...
	foreach($aPlugin as $plugin)
	{
		...
		$sql->query('SELECT `id` FROM `control__plugins_install` WHERE `plugin`="'.$plugin.'" AND `server`="'.$sid.'" LIMIT 1');

		if(!$sql->num())
			$sql->query('INSERT INTO `control__plugins_install` set `server`="'.$sid.'", `plugin`="'.$plugin.'", `time`="'.$copy['date'].'"');
	}
