<?php
	...
	$sql->query('SELECT `id`, `pack`, `name`, `info`, `plugins`, `date`, `status` FROM `control__copy` WHERE `id`="'.$cid.'" AND `user`="'.$ctrl['user'].'_'.$id.'" AND `game`="'.$server['game'].'" LIMIT 1');
	if(!$sql->num())
	...
	// Удаление плагинов
	$sql->query('DELETE FROM `control__plugins_install` WHERE `server`="'.$sid.'"');
	...
	foreach($aPlugins as $plugin)
	{
		...
		$sql->query('SELECT `id` FROM `control__plugins_install` WHERE `plugin`="'.$aPlugin[0].'" AND `server`="'.$sid.'" LIMIT 1');

		if(!$aPlugin[1])
			$aPlugin[1] = 0;

		if(!$sql->num())
			$sql->query('INSERT INTO `control__plugins_install` set `server`="'.$sid.'", `plugin`="'.$aPlugin[0].'", `upd`="'.$aPlugin[1].'", `time`="'.$copy['date'].'"');
	}

	// Очистка кеша
	$mcache->delete('ctrl_server_plugins_'.$sid);

	$sql->query('UPDATE `control__servers` set `status`="recovery" WHERE `id`="'.$sid.'" LIMIT 1');
