<?php
	...
	$sql->query('SELECT `id` FROM `control__copy` WHERE `server`="'.$sid.'" ORDER BY `id` DESC LIMIT 5');
	if($sql->num() > 4)
		sys::outjs(array('e' => 'Для создания новой копии необходимо удалить старые.'), $nmch);

	$sql->query('SELECT `id` FROM `control__copy` WHERE `server`="'.$sid.'" AND `status`="0" LIMIT 1');
	if($sql->num())
	...
	$sql->query('SELECT `plugin`, `upd` FROM `control__plugins_install` WHERE `server`="'.$sid.'"');
	while($plugin = $sql->get())
		$plugins .= $plugin['plugin'].'.'.$plugin['upd'].',';

	$sql->query('INSERT INTO `control__copy` set `user`="'.$ctrl['user'].'_'.$id.'", `game`="'.$server['game'].'", `server`="'.$sid.'", `pack`="'.$server['pack'].'", `name`="'.$name_copy.'", `info`="'.substr($info, 0, -2).'",  `plugins`="'.substr($plugins, 0, -1).'", `date`="'.$start_point.'", `status`="0"');
