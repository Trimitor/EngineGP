<?php
	...
	$sql->query('SELECT `uid`, `slots`, `map_start`, `vac`, `fastdl`, `autorestart`, `tickrate`, `core_fix`, `pingboost` FROM `control__servers` WHERE `id`="'.$sid.'" LIMIT 1');
	...
	if($go AND $url['save'])
	{
		...
		switch($url['save'])
		{
			...
			case 'address':
				...
				$sql->query('SELECT `id` FROM `control__servers` WHERE `unit`="'.$id.'" AND `address`="'.$address.'" LIMIT 1');
				if($sql->num())
					sys::outjs(array('e' => 'Данный адрес занят другим сервером'), $nmch);

				if($address != $server['address'])
					$sql->query('UPDATE `control__servers` set `address`="'.$address.'" WHERE `id`="'.$sid.'" LIMIT 1');
			...
			case 'mod':
				if(in_array($value, array(1, 2, 3, 4, 5)))
					$sql->query('UPDATE `control__servers` set `pingboost`="'.$value.'" WHERE `id`="'.$sid.'" LIMIT 1');
			...
			case 'vac':
				if($value != $server['vac'])
					$sql->query('UPDATE `control__servers` set `vac`="'.$value.'" WHERE `id`="'.$sid.'" LIMIT 1');
			...
			case 'core_fix':
				...
				if($value != $server['core_fix'])
					$sql->query('UPDATE `control__servers` set `core_fix`="'.$value.'" WHERE `id`="'.$sid.'" LIMIT 1');
			...
			case 'slots':
				...
				if($slots != $server['slots'])
					$sql->query('UPDATE `control__servers` set `slots`="'.$slots.'" WHERE `id`="'.$sid.'" LIMIT 1');
			...
			case 'autorestart':
				if($value != $server['autorestart'])
					$sql->query('UPDATE `control__servers` set `autorestart`="'.$value.'" WHERE `id`="'.$sid.'" LIMIT 1');
			...
			case 'tickrate':
				if(in_array($value, array('64', '128')))
					$sql->query('UPDATE `control__servers` set `tickrate`="'.$value.'" WHERE `id`="'.$sid.'" LIMIT 1');
			...
			case 'fastdl':
				...
				$sql->query('UPDATE `control__servers` set `fastdl`="'.$value.'" WHERE `id`="'.$sid.'" LIMIT 1');
