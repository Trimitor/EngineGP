<?php
	...
	if($go)
	{
		...
		$sql->query('DELETE FROM `control__admins_'.$server['game'].'` WHERE `server`="'.$sid.'"');
		...
		foreach($aData['value'] as $index => $val)
		{
			if($val != '')
			{
				...
				$sql->query('INSERT INTO `control__admins_'.$server['game'].'` set'
					.'`server`="'.$sid.'",'
	...
	$sql->query('SELECT `id`, `value`, `active`, `passwd`, `flags`, `immunity`, `time`, `info` FROM `control__admins_'.$server['game'].'` WHERE `server`="'.$sid.'" ORDER BY `id` ASC');
	while($admin = $sql->get())
	{
	...
	$sql->query('SELECT `id` FROM `control__admins_'.$server['game'].'` WHERE `server`="'.$sid.'" ORDER BY `id` DESC LIMIT 1');
	$max = $sql->get();
