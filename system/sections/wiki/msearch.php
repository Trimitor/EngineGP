<?php
	...
	$answer_q = $sql->query('SELECT `wiki` FROM `wiki__answer` WHERE `text` LIKE FROM_BASE64(\''.base64_encode('%'.$text.'%').'\') LIMIT 5');
	...
	if(!$sql->num($wiki_q) AND !$sql->num($answer_q) AND !isset($url['tag']))
	{
		...
		foreach($aText as $word)
		{
			...
			// Поиск по тексту (ответу)
			$answer_q = $sql->query('SELECT `wiki` FROM `wiki__answer` WHERE `text` LIKE FROM_BASE64(\''.base64_encode('%'.$word.'%').'\') LIMIT 5');
